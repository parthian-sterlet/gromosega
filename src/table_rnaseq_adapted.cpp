...
int main(int argc, char* argv[])
{
	...
	if (argc != 14)
	{
		...
		puts("7int #DEGmin,max (default 200) 8int #notDEGmin (default 1000) 9double threshold padj (default 0.05)");
	...
	int size_deg_min = atoi(argv[7]);	
	int size_nedeg_min = atoi(argv[8]);	
	...
	strcpy(fileo_sta, "table_rnaseq_adapted.txt");
	...
	for (i = 0; i < n_genes; i++)gen_type[i] = 10;
	char** genes;
	{
	...
	if (rev == 0)
	{		
		fprintf(out_sta, "%s\t%s\t%s\t%s\t%s\tTotal genes\t%d\t%s\t%d\t%s\t%d\t%s\t%d\tUpFC\t%f\tDoFC\t%f\tNoFC\t%f\t%f\n", filei_rnaseq, filei_genelist, argv[7], argv[8], argv[9], n_genes, argv[10], take_up, argv[11], take_do, argv[12], take_no, thr_log2fc_up, thr_log2fc_do, thr_log2fc_nemax, thr_log2fc_nemin);		
	}
	else
	{
		fprintf(out_sta, "%s\t%s\t%s\t%s\t%s\tReverse_up&down\t%d\tTotal genes\t%d\t%s\t%d\t%s\t%d\t%s\t%d\tUpFC\t%f\tDoFC\t%f\tNoFC\t%f\t%f\n", filei_rnaseq, filei_genelist, argv[7], argv[8], argv[9], rev, n_genes, argv[10], take_do, argv[11], take_up, argv[12], take_no, thr_log2fc_up, thr_log2fc_do, thr_log2fc_nemax, thr_log2fc_nemin);
	}
	fclose(out_sta);
	for (i = 0; i < n_genes; i++)
	{
