...
# Algorithm
Gromosega algorithm considers a pair of positive/negative sequence sets derived either from RNA-seq data. peaks. For RNA-seq data the positive and negative sequences are promoters of DEGs and non-DEGs, that are defined by the default criteria for adjusted p-value (padj) & Fold Change (FC) thresholds padj<sub>CRIT</sub> & log2(FC<sub>UP</sub>) / log2(FC<sub>DOWN</sub>): 
...
## GA input data:
- a pair of positive / negative sets, containing N<sub>POS</sub> / N<sub>NEG</sub> DNA sequences;
...
- internal structure of motif groups, a list of triangle matrices for M motif groups, computed separately for positive and negative sequence sets, each matrix contains M × (M - 1) / 2 Pearson's correlation coefficients for various pairs of -Log<sub>10</sub>(ERR) vectors representing separate motifs of the same group, the list of matrices is ranked in the descending order of the pAUPRC values;
- external structure of motif groups, two triangle matrices for all elite motifs computed separately for positive and negative sequence sets, each matrix contains M × (M - 1) / 2 Pearson's correlation coefficients for various pairs of -Log<sub>10</sub>(ERR) vectors representing the elite groups. 
