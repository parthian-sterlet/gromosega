...
## Python toolkit to visualize the GromoseGA results
[Python script](https://github.com/parthian-sterlet/gromosega/blob/main/src/gromosega-tools.zip) requires a separate installation, it work separately from the main package described above. It does not perform the search or classification purposes, it is required only to create heatmap of TF families and of TFBS motifs with logos, based on the ready collections of TFBS motifs from HOCOMOCO and JASPAR.
...
# Next applications
[Hocomoco](https://hocomoco.autosome.org/) database (version 14) provided the lists of [1594 human](https://github.com/parthian-sterlet/gromosega/blob/main/examples/rnaseq/hocomoco14_tfclass_hs1594) / [1245 mouse](https://github.com/parthian-sterlet/gromosega/blob/main/examples/rnaseq/hocomoco14_tfclass_mm1245) TFBS motifs classified by [TFClass database](https://doi.org/10.1093/nar/gkx987) into families, [Jaspar](https://jaspar.elixir.no/) database (version JASPAR2026) classified [869 plant](https://github.com/parthian-sterlet/gromosega/blob/main/examples/rnaseq/jaspar2026_tfclass_plants869)/[272 insect](https://github.com/parthian-sterlet/gromosega/blob/main/examples/rnaseq/jaspar2026_tfclass_insects272tbp) TFBS motifs according the [Plant-TFClass](https://doi.org/10.1016/j.tplants.2023.06.023) / [TFClass](https://doi.org/10.1093/nar/gkx987) databases, the filtration of these lists by [the approach described for MCOT package](https://doi.org/10.1093/nar/gkz800) requiring at least five distinct thresholds for each motif for the ERR threshold of 0.0025, we added to the list of insects the motif for [human TBP motif](https://hocomoco14.autosome.org/motif/TBP.H14CORE.0.P.B) since we proved earlier ([Zhimulev et al., 2024](https://doi.org/10.3390/ijms25074068)) its enrichment for Drosophila developmental genes (the state 'Ruby' of closed chromatin). 
